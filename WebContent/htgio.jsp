...
<body>
 ...
 -->
        ...
        <div class="app__container" style="min-height: 600px;">
        	<div class="container" >
			    	<table width="1000" style="padding: 20px 15px; margin: 0 auto 20px; font-size: 16px;" >
			    	<tr>
			    		<td></td>
			    		<td>Ảnh</td>
			    		<td>Tên đồng hồ</td>
			   		...
			   		<%	
			   			if (gh.ds.size() == 0){ %>
			   				<h2 style="display:block; text-align: center; font-size: 20px;padding-top: 20px;">Không có sản phẩm</h2>
						<%}
			   			if (gh.ds.size() != 0)
			   			...
			   			%>
			   				<tr class="row-item">
			   					<td>
				   					<input type="checkbox" name="delete" form="form-1" value="<%=item.getMaDongHo()%>"/>
				   				</td>
			   					<td>
				   				...
				   				<td>
				   					<form method="post" action="giohang?ms=<%=item.getMaDongHo()%>&tendongho=<%=item.getTenDongHo()%>&gia=<%=item.getGia()%>&anh=<%=item.getAnh()%>&addwatch=true&xuatxu=<%=item.getXuatXu()%>">
				   				...
				   				<td>
				   					<a class="test" href="giohang?ms=<%=item.getMaDongHo() %>&remove=1">&times</a>
				   				</td>
					...
					<a class="btn-action-1" href="htdongho">Tiếp tục mua hàng</a>
